...
ggplot(df, aes(x=group_good_time,fill = Health.issue.during.lockdown)) +
  ...
  labs(y="Percentage",x="Time on good habits (fitness, self study,online classes)",fill="Health.issue.during.lockdown?")  +
...
ggplot(df7, aes(x=group_screen,fill = Health.issue.during.lockdown)) +
  ...
  labs(y="Percentage",x="Hours of good habbit - =Hours of bad habbit",fill="Health issue during lockdown", caption = "Description: we take the total amount of time that spent on \ngood habits(fitness,self study and online class) and subtract the bad habits(time spent on social media and TV)")  + 
 theme(text = element_text(size=25),plot.caption = element_text(hjust = 0, face = "bold"))+ 
  scale_y_continuous(labels = scales::percent_format())
